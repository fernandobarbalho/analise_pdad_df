...
tempos_unidade_ensino<-  
  read_excel("dicionario_de_variaveis_pdad_2021.xlsx", 
                                                sheet = "moradores", skip = 325,n_max =11, col_names = FALSE) %>%
  select(3,4) 

names(tempos_unidade_ensino)<- c("h06","tempo_deslocamento")

meios_transporte<-
  read_excel("dicionario_de_variaveis_pdad_2021.xlsx", 
                                                sheet = "moradores", skip = 314,n_max = 9, col_names = FALSE) %>%
  select(3,4) 

names(meios_transporte)<- c("h05_10","meio_transporte")  
...
```{r }
#Gráfico da distribuição dos tempos para unidades de ensino
...
PDAD_2021_Moradores%>%
  ...
  summarise(proporcao_valido = ((round(sum(peso_mor),0))/peso_total_valido)*100,
            .by = h06) %>%
  inner_join(tempos_unidade_ensino) %>%
  arrange(h06) %>%
  mutate(tempo_deslocamento = reorder(tempo_deslocamento, desc(h06)),
  ...
  labs(
    title = "Tempos de deslocamento para educação no DF",
...
PDAD_2021_Moradores %>%
  ...
  geom_text(aes(label=paste0(proporcao,"%")), hjust=c(1,1,-3.5,-8), size=3)+
  theme_light()+
  theme(panel.grid = element_blank(),
        axis.text.x = element_blank()) +
  labs(
...
PDAD_2021_Moradores %>%
  mutate(mesma_regiao = ifelse(a01ra==h04,"sim","não")) %>%
  filter(!h06%in%c(88888,99999)) %>%
  filter(!(renda_ind %in% c(77777, 88888, 99999) )) %>%
  mutate(tempos_deslocamento = ifelse(h06<=2,"Até 30 minutos","Acima de 30 minutos")) %>%
  summarise( media_podenderada = weighted.mean(renda_ind, na.rm = TRUE),
             .by = c(mesma_regiao, tempos_deslocamento)) %>%
  arrange(desc(media_podenderada)) %>%
    ggplot(aes(y=mesma_regiao, x=media_podenderada)) +
  geom_col(aes(fill= tempos_deslocamento), position = "dodge") + 
  #geom_text(aes(label=paste0(proporcao,"%")), hjust=c(1,1,-3.5,-7), size=3)+
  theme_light()+
  theme(panel.grid = element_blank()) +
  labs(
...
PDAD_2021_Moradores %>%
  ...
  theme(panel.grid = element_blank()) +
  labs(
    title = "Tempos de deslocamento para educação no DF: a pé",
    subtitle = "Regiões administrativas/mesma região (s/n)",
    caption = "Fonte: IPE-DF. Dados de 2021. Elaboração própria",
    x="",
    y="",
...
PDAD_2021_Moradores %>%
  ...
  theme(panel.grid = element_blank()) +
  labs(
    title = "Tempos de deslocamento para educação no DF: automóvel",
    subtitle = "Regiões administrativas/mesma região (s/n)",
    caption = "Fonte: IPE-DF. Dados de 2021. Elaboração própria",
    x="",
    y="",
...
PDAD_2021_Moradores %>%
  ...
  theme(panel.grid = element_blank()) +
  labs(
    title = "Tempos de deslocamento para educação no DF: Sudoeste e Octogonal",
...
PDAD_2021_Moradores %>%
  ...
  theme(panel.grid = element_blank()) +
  labs(
    title = "Tempos de deslocamento para educação no DF: Ceilândia",
    subtitle = "Meio de transporte/mesma região (s/n)",
    caption = "Fonte: IPE-DF. Dados de 2021. Elaboração própria",
    x="",
    y="",
...
```{r fig.height=8}
#Gráfico da distribuição dos tempos para o educação por região administrativa
